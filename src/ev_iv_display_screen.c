...
static void UpdateCurrentMon(void);//new
static void Task_EvIvReturn(u8);
static void BufferMonData(struct Pokemon * mon);
static s8 AdvanceMultiBattleMonIndex(s8 direction);//new
static void ShowSprite(struct Pokemon *mon);
static void EvIvPrintText(struct Pokemon *mon);
static void ShowPokemonPic2(u16 species, u32 otId, u32 personality, u8 x, u8 y);//PrintPokemonSpriteEvIv
static void Task_ScriptShowMonPic(u8 taskId);
static void HidePokemonPic2(u8 taskId);//cambiar name a HidePokemonSpriteEvIv
 ...
 * 2022-03-03
 * ACIMUT:
 * Esta función configura el ev-iv display screen por 
