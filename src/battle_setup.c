...
static void DoStandardWildBattle(void)
{
    ...
    if (gBattleTypeFlags & BATTLE_TYPE_CHANSEY)
    {
        ...
        GivePowerToMon(&gEnemyParty[0], 0, (MAX_PER_STAT_EVS - 100), 10, MAX_PER_STAT_IVS);
        song = MUS_BW_WILD_BATTLE;
    }
    else if (gBattleTypeFlags & BATTLE_TYPE_POWER_WILD)
    {
        GivePowerToMon(&gEnemyParty[0], 50, (MAX_PER_STAT_EVS - 100), 15, MAX_PER_STAT_IVS);
